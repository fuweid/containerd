...
func (u *unpacker) unpack(
	...
	doUnpackFn := func(i int, desc ocispec.Descriptor) error {
		...
		unlock, err := u.lockChainID(ctx, chainID)
		if err != nil {
			return err
		}
		defer unlock()
...
func (u *unpacker) fetch(ctx context.Context, h images.Handler, layers []ocispec.Descriptor, done []chan struct{}) error {
	...
	for i, desc := range layers {
		...
		if err := u.doAcquire(ctx); err != nil {
			return err
		}

		eg.Go(func() error {
			unlock, err := u.lockDescriptor(ctx2, desc)
			if err != nil {
				u.doRelease()
				return err
			}
...
func (u *unpacker) handlerWrapper(
	...
	return func(f images.Handler) images.Handler {
		...
		return images.HandlerFunc(func(ctx context.Context, desc ocispec.Descriptor) ([]ocispec.Descriptor, error) {
			unlock, err := u.lockDescriptor(ctx, desc)
			if err != nil {
				return nil, err
			}
...
func (u *unpacker) doAcquire(ctx context.Context) error {
	if u.limiter == nil {
		return nil
	}
...
func (u *unpacker) lockChainID(ctx context.Context, chainID string) (func(), error) {
...
func (u *unpacker) lockDescriptor(ctx context.Context, desc ocispec.Descriptor) (func(), error) {
	key := u.makeDescriptorKey(desc)
...
func (u *unpacker) makeDescriptorKey(desc ocispec.Descriptor) string {
